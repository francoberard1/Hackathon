...
def validate_receipt_review_submission(form, members: list[dict]) -> dict:
    ...
    enabled_items = _build_enabled_items(
        form.getlist('item_name[]'),
        form.getlist('item_amount[]'),
        form.getlist('item_user_id[]'),
        form.getlist('item_enabled[]'),
        member_ids,
    )
    ...
    if total_amount != computed_total:
        raise ReceiptReviewValidationError("Total must equal items + tax + tip after your edits.")

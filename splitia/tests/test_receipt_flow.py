...
class ReceiptRouteTests(unittest.TestCase):
    ...
    def test_receipt_page_renders(self):
        response = self.client.get(f"/add_expense/{self.group_id}/receipt")

        self.assertEqual(response.status_code, 200)
        self.assertIn(b"Add Expense From Receipt Photo", response.data)
    ...
    def test_manual_add_expense_still_uses_equal_split(self):
        response = self.client.post(
